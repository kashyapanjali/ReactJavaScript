...
export default function EmployeData() {
	...
	const handleUpdate = () => {
		...
		if (index !== -1) {
			const dt = [...data];
			dt[index] = { id, firstName, lastName, age, role };
			setData(dt);
			handleClear();
		}
	};
			...
			<div className='container'>
				...
				<div className='d-flex flex-column align-items-center'>
					<div className='row justify-content-center mb-4 w-100'>
						...
						<div className='col-12 col-md-3 my-2'>
							<label>
								<div className='text-center text-info'>Role</div>
								<input
									type='text'
									className='form-control rounded-pill'
					...
					<div className='d-flex justify-content-center mb-4'>
						{!isUpdate ? (
							<button
								className='btn border-dark btn-primary'
								onClick={handleSave}>
								Save
							</button>
						) : (
						...
						<button
							className='btn border-dark btn-warning mx-1 text-white'
							onClick={handleClear}>
							Clear
						</button>
					</div>
						...
						<tbody>
							...
							{data.map((item, index) => (
								<tr key={index}>
									...
									<td>
										<button
											className='btn btn-primary border-dark mx-1 my-1'
